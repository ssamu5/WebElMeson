    ...
    <footer className="bg-dark-secondary border-t border-dark-border mt-20">
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-3">
              <div className="relative w-12 h-12">
                <Image src="/images/logo-sm.webp" alt="El Mesón Smashburgers" fill className="object-contain" sizes="48px" />
              </div>
              <div>
                <p className="font-display text-xl text-[#f0ece4] uppercase tracking-wider">El Mesón</p>
                <p className="text-xs text-muted uppercase tracking-widest">Smashburgers</p>
              </div>
            </div>
            <p className="text-muted text-sm leading-relaxed">
              Smash burgers artesanales con ingredientes frescos y recetas únicas en La Ribera de Navarra.
            </p>
            <a href={SITE_CONFIG.instagram.url} target="_blank" rel="noopener noreferrer"
          ...
          {/* Navigation */}
          <div>
            <p className="font-display text-sm text-[#f0ece4] uppercase tracking-widest mb-4">Navegación</p>
            <ul className="flex flex-col gap-2">
              {[
                ...
                <li key={link.href}>
                  <Link href={link.href} className="text-muted hover:text-brand-pink transition-colors duration-200 text-sm">
                    {link.label}
                  </Link>
                </li>
              ))}
          ...
          {/* Contact */}
          <div>
            <p className="font-display text-sm text-[#f0ece4] uppercase tracking-widest mb-4">Contacto</p>
            <ul className="flex flex-col gap-3 text-sm">
              <li>
                <a href={SITE_CONFIG.phone.localHref} className="flex items-center gap-2 text-muted hover:text-brand-pink transition-colors duration-200">
                  <PhoneIcon />{SITE_CONFIG.phone.local}
                </a>
              </li>
              <li>
                <a href={SITE_CONFIG.emailHref} className="flex items-center gap-2 text-muted hover:text-brand-pink transition-colors duration-200">
                  <MailIcon />{SITE_CONFIG.email}
                </a>
              </li>
              <li className="flex items-start gap-2 text-muted">
                <MapIcon className="shrink-0 mt-0.5" /><span>{SITE_CONFIG.address}</span>
              </li>
              <li className="flex items-center gap-2 text-muted">
                <ClockIcon />{SITE_CONFIG.hours}
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="mt-10 text-center">
          <p style={{ color: "#B53863", opacity: 0.5, fontSize: "0.8rem", letterSpacing: "0.4em" }}>
            ᚱ ᚢ ᚾ ᛟ ᚱ
          </p>
        </div>

        {/* Bottom bar */}
        <div className="mt-4 pt-4 border-t border-dark-border flex flex-col md:flex-row items-center justify-between gap-4">
...
function MapIcon({ className }: { className?: string }) {
  return <svg className={className} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>;
}
function ClockIcon() {
